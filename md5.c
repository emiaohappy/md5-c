...
static const uint32_t S[] = {7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17,
 ...
 */
void
md5Update(MD5Context *ctx, const uint8_t *input_buffer, size_t input_len) {
    uint32_t input[16];
    unsigned int offset = ctx->size % 64;
    ctx->size += (INPUT_SIZE_T) input_len;
...
md5Finalize(MD5Context *ctx) {
    ...
    ctx->size -= (INPUT_SIZE_T) padding_length;
    ...
    input[14] = (uint32_t)(ctx->size << 3);
    input[15] = (uint32_t)(ctx->size >> 29);
 ...
 */
void
md5String(char *input, uint8_t *result) {
    MD5Context ctx;
    md5Init(&ctx);
...
void
md5File(fat_file_t *file, uint8_t *result) {
#define INPUT_BUFFER_SIZE (1024)
    ...
    while ((input_size =
                    fat_file_read(file, input_buffer, INPUT_BUFFER_SIZE, NULL)) > 0) {
